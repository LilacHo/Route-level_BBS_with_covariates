// Route- and abundance-varying zero-inflated negative-binomial test model.
//
// This extends ZINB_route_test by allowing structural-zero probability to
// depend on the expected log abundance E[i]. This is intended for species where
// route-level zero inflation improves fit but still underpredicts zeros:
//
//   logit(zi_obs[i]) = zi_intercept
//                      + sd_zi_route * zi_route_raw[route[i]]
//                      + zi_log_mu * E[i]
//
// A negative zi_log_mu means observations with low expected abundance have
// higher structural-zero probability.
...
transformed parameters {
  ...
  real phi;
  vector[ncounts] E;
  vector<lower=0, upper=1>[ncounts] zi_obs;
  ...
  for (i in 1:ncounts) {
    E[i] = alpha[route[i]]
      ...
      + obs[observer[i]]
      + eta * firstyr[i];
    zi_obs[i] = inv_logit(zi_intercept
      + sd_zi_route * zi_route_raw[route[i]]
      + zi_log_mu * E[i]);
  }
}
...
model {
  ...
  for (i in 1:ncounts) {
    target += zinb2_log_lpmf(count[i] | E[i], phi, zi_obs[i]);
  }
}
...
generated quantities {
...

  {
    ...
    for (i in 1:ncounts) {
      log_lik[i] = zinb2_log_lpmf(count[i] | E[i], phi, zi_obs[i]);
    }
