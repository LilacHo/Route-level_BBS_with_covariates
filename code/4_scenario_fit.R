...
here::i_am("code/4_scenario_fit.R")
...
# Scenario assignments produced by code/3_assign_model_scenarios.R. If this
...
route_summary_from_fit <- function(summ, route_map, species, species_code,
  ...
  extract_route_vec <- function(prefix) {
    summ %>%
      filter(str_detect(variable, paste0("^", prefix, "\\["))) %>%
      transmute(routeF = as.integer(str_extract(variable, "\\d+")),
                !!prefix := mean)
  }

  ch_route <- extract_route_vec("CH_route")

  if (nrow(ch_route) > 0) {
    # Preferred path: use the per-route change in expected observed count that
    # the Stan model now reports directly. This puts CH_no_habitat on the same
    # estimand (trend in the marginal mean) across every model scenario,
    # including the abundance-varying zero inflation in ZINB_route_mu.
    ch_df <- ch_route %>%
      left_join(extract_route_vec("CH_no_habitat_route"), by = "routeF") %>%
      left_join(extract_route_vec("CH_dif_route"), by = "routeF") %>%
      transmute(
        routeF,
        CH = CH_route,
        CH_no_habitat = CH_no_habitat_route,
        CH_dif = CH_dif_route
      )
  } else {
    # Fallback for older fits saved before the models exported CH_route:
    # reconstruct CH from the route-level slopes on the NB mean scale.
    warning("Fit for ", species_code, " lacks CH_route; falling back to ",
            "beta-based CH (NB-mean scale, not marginal). Refit to get the ",
            "consistent marginal-mean estimand.")
    beta_df <- summ %>%
