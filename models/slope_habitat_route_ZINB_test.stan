// Zero-inflated negative-binomial test variant for abundant/patchy species.
// This adds one global structural-zero probability to the improved NB test
// model. It is intentionally simple so the first test isolates whether excess
// zeros are the main posterior predictive problem.
...
model {
  ...
  for (i in 1:ncounts) {
    target += zinb2_log_lpmf(count[i] | E[i], phi, zi);
  }
}
...
generated quantities {
...

  {
    ...
    for (i in 1:ncounts) {
      log_lik[i] = zinb2_log_lpmf(count[i] | E[i], phi, zi);
    }
