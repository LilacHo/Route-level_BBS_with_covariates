# 7_statistical_analysis_and_visualization.R
#
...
here::i_am("code/7_statistical_analysis_and_visualization.R")

# Settings -----------------------------------------------------------------
land_cover <- "grasslands"   # target group: only SDM CSVs named
                              # <land_cover>_<species>_route_CH_sdm.csv are
                              # included in the combined analysis and plots
...
if (length(tierA_present) == 0) {
  ...
  run_category_tests(analysis_45_A, "CH_no_habitat", "category", "RCP 4.5", "Residual")
  run_category_tests(analysis_85_A, "CH_no_habitat", "category", "RCP 8.5", "Residual")
  ...
  make_violin_cld(
    ...
    x_lab = "SDM classified change category",
    file_name = paste0(tierA_tag, "_ALL_species_CH_no_habitat_by_rcp_cld.png")
  )
  ...
  run_category_tests(analysis_45_A_grp, "CH_no_habitat", "change_group", "RCP 4.5", "Residual")
  run_category_tests(analysis_85_A_grp, "CH_no_habitat", "change_group", "RCP 8.5", "Residual")
  ...
  make_violin_cld(
    ...
    subtitle = paste0(nrow(tierA_sdm), " route-species observations across ",
                      ...
                      length(tierA_present), " converged species"),
    x_lab = "Range change group",
    file_name = paste0(tierA_tag, "_ALL_species_CH_no_habitat_by_group_cld.png")
  )
}
