...
here::i_am("code/3_assign_model_scenarios.R")
...
# When baseline diagnostics are missing (e.g. a fresh run or the end-to-end
# subset test), degrade gracefully instead of stopping: every species is
# assigned the default NB_test scenario. classify_scenario() returns NB_test
# when all diagnostic inputs are NA, so empty diagnostic tables achieve this.
have_baseline_diagnostics <- !is.na(conv_file) && !is.na(ppc_file)
if (!have_baseline_diagnostics) {
  warning("Could not find baseline convergence and PPC diagnostics in ", diag_dir,
          ". Assigning the default NB_test scenario to all species. Run ",
          "diagnostics on the baseline model first to enable scenario ",
          "selection.")
}
...
conv <- if (have_baseline_diagnostics) {
  read.csv(conv_file) %>%
    transmute(
      species_code,
  ...
  tibble(
    ...
    baseline_max_rhat = numeric(),
    baseline_min_ess_bulk = numeric(),
    baseline_min_ess_tail = numeric(),
    baseline_worst_rhat_param = character()
  )
}

ppc <- if (have_baseline_diagnostics) {
  read.csv(ppc_file) %>%
    transmute(
      species_code,
      obs_mean,
...
summary_lines <- c(
  ...
  if (have_baseline_diagnostics) {
    c(paste("Convergence diagnostics:", conv_file),
      paste("PPC diagnostics:", ppc_file))
  } else {
    c("Convergence diagnostics: NONE FOUND (defaulted all species to NB_test)",
      "PPC diagnostics: NONE FOUND (defaulted all species to NB_test)")
  },
  "",
  "Assignment counts:",
